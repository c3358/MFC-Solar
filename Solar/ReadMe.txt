================================================================================
    MICROSOFT 基础类库 : Solar 项目概述
===============================================================================

应用程序向导已为您创建了此 Solar 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。

本文件概要介绍组成 Solar 应用程序的每个文件的内容。
...
Solar.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CSolarApp 应用程序类。

Solar.cpp
    这是包含应用程序类 CSolarApp 的主应用程序源文件。

Solar.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\Solar.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 Solar.rc 中。
...
res\Toolbar.bmp
    此位图文件用于为工具栏创建平铺图像。
    初始工具栏和状态栏在 CMainFrame 类中构造。使用资源编辑器编辑此工具栏位图，并更新 Solar.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加工具栏按钮。
/////////////////////////////////////////////////////////////////////////////

应用程序向导创建一种文档类型和一个视图：

SolarDoc.h、SolarDoc.cpp - 文档
    这些文件包含 CSolarDoc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 CSolarDoc::Serialize）。
    该文档将包含以下字符串：
        文件扩展名：      solar
        文件类型 ID：        Solar.Document
        主框架标题：  Solar System
        文档类型名称：       Solar
        筛选器名称：         Solar Files (*.solar)
        文件的新短名称：Solar
        文件类型长名称：Solar.Document

SolarView.h、SolarView.cpp - 文档视图
    这些文件包含 CSolarView 类。
    CSolarView 对象用于查看 CSolarDoc 对象。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 Solar.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
